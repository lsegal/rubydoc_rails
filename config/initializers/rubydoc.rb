...
module Rubydoc
  ...
  self.config = Rails.application.config_for(:rubydoc)
end
...
YARD::Templates::Engine.template(:default, :fulldoc, :html).full_paths.each do |path|
  %w[css js images].each do |ext|
    srcdir, dstdir = File.join(path, ext), Rails.root.join("public", "assets", ext)
    next unless File.directory?(srcdir)

    FileUtils.mkdir_p(dstdir)
    FileUtils.cp_r(Dir.glob(File.join(srcdir, "*")), dstdir)
  end
end
